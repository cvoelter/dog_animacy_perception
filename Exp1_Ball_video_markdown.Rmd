...
pupil.group.level.raw <- sample.data %>%
      filter(!(subject=="subj.03" & trial==4), !(subject=="subj.14" & trial==4))%>%
...
plot.reversed_videos_ball.pupil.raw <- ggplot(data = pupil.group.level.raw%>%filter(!is.na(condition)), aes(x = time.frame, y = mean.pupil)) +
...
plot.reversed_videos_ball.pupil.raw
...
exp.data.pupil.processed <- sample.data %>%
  ...
  ungroup() %>%
  group_by(subject, sex, session, condition_trial) %>%
...
dat <- exp.data.pupil.processed.downsampled%>%
    filter(!(subject=="subj.03" & trial==4), !(subject=="subj.14" & trial==4))%>% #missing data in one condition in the last trial
...
pg_s2<-plot_grid(plot.reversed_videos_ball.pupil.raw, pg_s1, nrow=2, rel_heights=c(1.5,1), labels=c("A", ""))

ggsave("graphics/reversed_videos_ball_video_pupil_size_sup_info.png", width=11, height=11, scale=0.7)
...
save.image(file = "reversed_videos_ball_gamm_workspace_rev2.RData")
```
...
plot_diff_no_plot<-plot_diff(m2, view="time", 
          comp=list(condition=c("normal", "reversed")), rm.ranef=TRUE, hide.label = TRUE, plot = FALSE)
...
plot_diff_m2_gg<-ggplot(data=plot_diff_no_plot, aes(x=time, y=est))+
  ...
  geom_vline(xintercept = x$start[2], lty=3, color="red")+
  geom_vline(xintercept = x$end[2], lty=3, color="red")+
...
```{r}
png("graphics/plot_reversed_videos_ball_m2_paterial_effect.png", width=20, height=18, units="cm", res=300)
plot(m2, pages = 1, all.terms = TRUE, residuals = TRUE, rug=FALSE, pch = 1, cex = 0.5, seWithMean = TRUE, shade = TRUE, shade.col = "lightblue")
dev.off()

library(mgcViz)
b <- getViz(m2)
png("graphics/plot_reversed_videos_ball_m2_paterial_effect2.png", width=20, height=18, units="cm", res=300)
p1<-plot(b, allTerms = T, select=c(1:6, 8))+theme_classic() + labs(title = NULL)
plot(b)
...
* Summed effects
```{r}
png("graphics/plot_reversed_videos_ball_m2_summed_effect.png", width=14, height=10, units="cm", res=600)
#
...
plot_smooth(m2, view="time", cond=list(condition="reversed"), rm.ranef=TRUE,
  v0=0, col="dodgerblue", lwd=2, lty=6, rug=FALSE, se=1.96, las=1,add=TRUE,
  ylim=c(-1000,1000), xlim=c(0, 5000))
...
plot_reversed_videos_ball_gaze_horizontal<-ggplot(data = gaze_plot_data%>%filter(!is.na(condition)), aes(x = time.frame, y = mean.x)) +
  ...
  geom_ribbon(data = gaze_plot_data %>% filter(condition=="reversed"), aes(x = time.frame, ymin = x_l_reversed, ymax = x_r_reversed), fill = "darkblue", alpha = 0.8) +
    geom_ribbon(data = gaze_plot_data %>% filter(condition=="normal"), aes(x = time.frame, ymin = x_l_normal, ymax = x_r_normal, fill = condition), alpha = 0.8) +
# gaze data:
  ...
  theme(panel.grid.major.x = element_blank(),
    ...
    legend.background = element_rect(fill = "transparent"))+
  scale_fill_manual(
    values = c("darkblue", "#5a5e63", "black"),
    name = "",
    breaks = c("normal", "x", "y"),

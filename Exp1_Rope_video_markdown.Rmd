...
load("reversed_videos_rope_gamm_workspace_rev.RData")
```
...
plot.reversed_videos_rope.pupil.raw <- ggplot(data = pupil.group.level.raw%>%filter(!is.na(condition)), aes(x = time.frame, y = mean.pupil)) +
...
plot.reversed_videos_rope.pupil.raw
...
pg_s2<-plot_grid(plot.reversed_videos_rope.pupil.raw, pg_s1, nrow=2, rel_heights=c(1.5,1), labels=c("A", ""))

ggsave("graphics/reversed_videos_rope_video_pupil_size_sup_info.png", width=11, height=11, scale=0.7)
...
save.image(file = "reversed_videos_rope_gamm_workspace_rev.RData")
```
...
```{r}
png("graphics/plot_reversed_videos_rope_m2_paterial_effect.png", width=20, height=18, units="cm", res=300)
plot(m2, pages = 1, all.terms = TRUE, residuals = TRUE, rug=FALSE, pch = 1, cex = 0.5, seWithMean = TRUE, shade = TRUE, shade.col = "lightblue")
dev.off()

library(mgcViz)
b <- getViz(m2)
png("graphics/plot_reversed_videos_rope_m2_paterial_effect2.png", width=20, height=18, units="cm", res=300)
...
* Summed effects
```{r}
png("graphics/plot_reversed_videos_rope_m2_summed_effect.png", width=16, height=12, units="cm", res=600)
...
plot_smooth(m2, view="time", cond=list(condition="reversed"), rm.ranef=TRUE,
  v0=0, col="dodgerblue", lwd=2, lty=6, rug=FALSE, se=1.96,
  main="", ylab="Pupil size (baseline corrected)", las=1,add=TRUE,
  ylim=c(-1000,1000), xlim=c(00, 5000))
# legend
legend('bottomright',
...
# gaze data:
  ...
  scale_y_reverse() +
  scale_fill_manual(
    values = c("darkgoldenrod1", "#5a5e63", "black"),
    name = "",
    breaks = c("normal", "x", "y"),
    labels = c("Rope", "Mean ± SE", "Median")
  )

...
#load("reversed_videos_gamm_workspace.RData")
```
...
#   geom_vline(aes(xintercept=4000), lty=1, col="darkgrey")+
 #  geom_vline(aes(xintercept=5000), lty=2, col="darkgrey")+
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        legend.title = element_blank(), legend.position = c(0.25, 0.8), legend.text=element_text(size=12))
...
plot_diff_no_plot<-plot_diff(m2, view="time", 
          comp=list(condition=c("normal", "reversed")), rm.ranef=TRUE, main="Disappear - Reappear", 
          las=1, ylab="Est. difference in pupil size", 
...
```{r}
png("graphics/plot_reversed_videos_cube_m2_paterial_effect.png", width=20, height=16, units="cm", res=300)
plot(m2, pages = 1, all.terms = TRUE, residuals = TRUE, rug=FALSE, pch = 1, cex = 0.5, seWithMean = TRUE, shade = TRUE, shade.col = "lightblue")
dev.off()

library(mgcViz)
b <- getViz(m2)
png("graphics/plot_reversed_videos_cube_m2_paterial_effect2.png", width=20, height=16, units="cm", res=300)
...
plot_reversed_videos_cube_gaze<-ggplot(data = gaze_plot_data%>%filter(!is.na(condition)), aes(x = time.frame, y = mean.y)) +
  ...
  geom_ribbon(data = gaze_plot_data %>% filter(condition=="reversed"), aes(x = time.frame, ymin = y_up_reversed, ymax = y_low_reversed), fill = "darkgreen", alpha = 0.8) +
    geom_ribbon(data = gaze_plot_data %>% filter(condition=="normal"), aes(x = time.frame, ymin = y_up_normal, ymax = y_low_normal), fill = "darkgreen", alpha = 0.8) +
# gaze data:
